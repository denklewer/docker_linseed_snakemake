    ...
    for ct in range(options.min_ct,options.max_ct+1):
        ...
        if options.local:
            ...
            cmd = """ P_WD=`pwd`; mkdir -p "$P_WD/tmp";
                ...
                bsub -cwd $HOME -n $L_CORES -G compute-martyomov -q general -oo $P_LOG -R 'span[hosts=1]' -a "docker($SMK_DOCKER_IMG)" /usr/bin/script -fqe /dev/null  -c "source /etc/bash.bashrc; cd $P_WD; export TMPDIR=$P_WD/tmp; echo $__LSF_JOB_CUSTOM_TMPDIR__; {0}" """.format(snk_cmd,
